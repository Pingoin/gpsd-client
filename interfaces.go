...
type GPSD struct {
	SatsVisible         []SatInfo `json:"satsVisible"`
...
type SatInfo struct {
	PRNNumber        uint8   `json:"prn"`              // SV PRN number, pseudo-random noise or gold code
	...
	Freqid           uint8   `json:"freqid"`           // For GLONASS satellites only: the frequency ID of the signal. As defined by u-blox, range 0 to 13. The freqid is the frequency slot plus 7.
	Health           uint8   `json:"health"`           // The health of this satellite. 0 is unknown, 1 is OK, and 2 is unhealthy.
}

// Satellite describes a location of a GPS satellite
type Satellite struct {
	PRN    uint8   `json:"PRN"`    // PRN ID of the satellite. 1-63 are GNSS satellites, 64-96 are GLONASS satellites, 100-164 are SBAS satellites
